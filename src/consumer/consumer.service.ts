...
export class ConsumerService {
  constructor(@InjectModel(Consumer.name) private consumerModel: Model<Consumer>,@InjectModel(City.name) private cityModel: Model<City>,@InjectModel(State.name) private stateModel: Model<State>,@InjectModel(Country.name) private countryModel: Model<Country>,@InjectModel(User.name) private userModel: Model<User>) { }
  ...
  async createConsumer(
    ...
    // Check if a consumer already exists for the user
    const existingConsumer = await this.consumerModel.findOne({ user: userId });

    if (existingConsumer) {

      throw new HttpException('Consumer details already exist for this user',HttpStatus.BAD_REQUEST);
    ...
    if (!city || !state || !country) {
      ...
      throw new HttpException('Invalid city, state, or country',HttpStatus.BAD_REQUEST,);
    }
  ...
  // Get a consumer by user ID
  async getConsumerById(
    userId: string,
  ): Promise<{ success: true; consumer: Consumer }> {
    const consumer = await this.consumerModel.findOne({ user: userId }).populate('user').exec();
  ...
  // Update a consumer
  async updateConsumer(userId: string,updateConsumerDto: UpdateConsumerDto): Promise<{ success: true; consumer: Consumer }> {
    
    const existingConsumer = await this.consumerModel.findOne({ user: userId }).populate('user').exec();
    ...
    if (updateConsumerDto.country) {
      ...
      const country = await this.countryModel.findById(updateConsumerDto.country);
  ...
  // Delete a consumer
  async deleteConsumer(userId: string,): Promise<{ success: true; message: string }> {

    const existingConsumer = await this.consumerModel.findOneAndDelete({ user: userId }) .populate('user').exec();
